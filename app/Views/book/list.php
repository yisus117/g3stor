      ...
      <div class="table-title gap-4" style="outline: 3px solid <?= config("G3stor")->mainColor ?> ;background-color: <?= config("G3stor")->secondColor ?>">
        ...
        <div class="row">
          <div class="col-6  d-flex align-items-center justify-content-between">
            <a href="<?= base_url(route_to("books")) ?>" class="title-hover no_wrap d-inline h3 text-sm-center text-md-start text-white">
              Libros
          ...
          <div class="col-6  ">
            <a href="<?= base_url(route_to("books_create")) ?>" class="btn btn-secondary p-2 px-3 mt-1 d-flex align-items-center gap-2 " style="color: white; background-color: <?= config("G3stor")->mainColor ?>" title="Agregar un nuevo registro">
              ...
              <div class="search row d-flex justify-content-center align-items-center gap-2">
                <div class="col-6 col-md-3 m-0 pe-0">
                  <select name="field" class="form-select border-0 fw-semibold " required style="background-color: <?= config("G3stor")->grayColor ?>; color: <?= config("G3stor")->textColor ?>;">
                    <option selected hidden value="isbn">ISBN</option>
                ...
                <div class="col-8 col-md-6 m-0 p-0">
                  <div class="d-flex justify-content-around  ">
                    <input name="q" value="<?= $query ?? '' ?>" type="text" class="form-control fw-semibold" placeholder="Buscar libro" style="background-color: <?= config("G3stor")->grayColor ?>; color: <?= config("G3stor")->textColor ?>;">
          ...
          <tbody>
            ...
            <?php foreach ($books as $book) : ?>
              ...
              <tr>
                ...
                <td><?= $book->isbn ?></td>
                <td><?= $book->nombre ?></td>
                <td><?= $book->getEditorial()->Nombre ?> </td>
                <td><?= $book->edicion ?> </td>
                <td><?= $book->paginas ?> </td>
                <td><?= number_format($book->costo, 0) ?> </td>
                <td>
                  <span class=" px-2 py-1 fw-bold  <?= $book->activo == "1" ? "bg-success text-white text-nowrap" : ($book->activo === "2" ? "bg-warning text-dark text-nowrap" : "bg-danger") ?>">
                  <?= $book->activo == "1" ? "activo" : ($book->activo === "2" ? "Inactivo" : ($book->activo === "3" ? "En prestamo" : "Eliminado")) ?>
                  </span>
                </td>
                <td class="">
                  <div class="d-flex f-row gap-2 mx-0 ">
                    <a href="<?= $book->getEditLine($book->id_libros) ?>" name="update" class="btn btn-secondary text-white update px-2 py-1 h-4 d-flex justify-content-center align-items-center">
                      <i class="fa-solid fa-edit fs-6"></i>
                    </a>
                    <button type="button" data-name="<?= $book->Nombre ?>" class="btn btn-danger delete text-white px-2 py-1">
